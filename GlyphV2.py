...
def beatmap_info(file):
    ret = []
    map = osuparser.BeatMapSet(f"temp-files/{file}")
    for c in range(len(map.beatmaps)):
        meta = map.beatmaps[c].get_metadata()
        meta.update({'n': c})
        ret.append(meta)
...
def get_audio(file, n, audio_name):
    map = osuparser.BeatMapSet(f"temp-files/{file}")
    f_name = map.get_audio(map.beatmaps[n], name=f"temp-files/{audio_name}")
    return f_name


def get_map_w(file, n):
    map = osuparser.BeatMapSet(f"temp-files/{file}")
    return map.beatmaps[n]


def conv(map, ns, seed, mode):
    messages = []
    meta = map.get_metadata()
    messages.append({
        'type': 'info',
        'message': f"Map selected: {meta['artist']} - {meta['title']} [{meta['version']} | by {meta['creator']}] ({meta['objects_count']} objects)",
        'color': 'white'
    })
    label = map_to_label(map, ns, seed, mode)
    messages.append({
        'type': 'info',
        'message': f"Wrote {len(label)} lines",
        'color': 'white'
    })
